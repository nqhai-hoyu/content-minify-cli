...
async function protectJavaScriptSource(
  source,
  { module = false, obfuscate = false } = {},
) {
  ...
  if (!obfuscate) return minified.code;
...
async function protectJavaScriptFile(filePath, { obfuscate = false } = {}) {
  const source = await readFile(filePath, "utf8");
  let protectedCode;
  try {
    protectedCode = await protectJavaScriptSource(source, { obfuscate });
  } catch (error) {
...
async function protectHtmlFile(filePath, { obfuscate = false } = {}) {
  const source = await readFile(filePath, "utf8");
...
export async function protectContent(outputRoot, { obfuscate = false } = {}) {
  const files = await listFiles(outputRoot);
  const { javascriptFiles, cssFiles, htmlFiles } = classifyFiles(files);

  for (const filePath of javascriptFiles) {
    await protectJavaScriptFile(filePath, { obfuscate });
  }
  for (const filePath of cssFiles) await protectCssFile(filePath);
  for (const filePath of htmlFiles) await protectHtmlFile(filePath, { obfuscate });
}
